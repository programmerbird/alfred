...
PRE_VIDEO_PATTERN = re.compile(r'\[vimeo:([^\]]+)\]')
VIDEO_LINKS = PRE_VIDEO_LINKS + (
	(PRE_VIDEO_PATTERN, vimeo),	
)
...
def get_thumbnail(txt):
	...
	return "http://vimeo.com/api/v2/video/%(videoid)s.output" % locals()
