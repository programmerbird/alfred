...
def shortdate(value):
	try:
		...
		if display=='12:00 am':
			display = 'Today'
		q = '<abbr class="time" title="%s">%s</abbr>' % (value.strftime("%b %d, %Y @ %l:%M%P"), display)
		return mark_safe(q)	
	except:
		return ''	
...
def numberformat(value, format="%.0-2f"):
	from ..hibird.utils import comma_number
	return comma_number(value, format=format)
