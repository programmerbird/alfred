...
CHUNK_SIZE = 100
def clear_nullable_related(instance):
	for related in instance._meta.get_all_related_objects():
		accessor = related.get_accessor_name()
		related_set = getattr(instance, accessor)
		if related.field.null:
			related_set.clear()
		else:
			for related_object in related_set.all():
				clear_nullable_related(related_object)

def bind_clear_nullable_related(model_class):
	original_delete = model_class.delete
	def new_delete(self):
		clear_nullable_related(self)
		original_delete(self)
	model_class.add_to_class('delete', new_delete)
